...
### 2.1. Module `free_list_allocator.sv` (Quản lý Bộ nhớ Động O(1))
...
### 2.2. Module `hierarchical_bitmap_ffs.sv` (Cây Bitmap Tìm Top-of-Book O(1))
- **Vấn đề cần giải quyết:** Thay vì duyệt cây Binary Heap mất 10–20 chu kỳ, làm sao tìm ra mức giá cao nhất/thấp nhất trong 1 chu kỳ?
- **Kiến trúc mạch:**
...
- **Ngân sách trễ logic (Timing Budget):**
  - Trễ Tầng 0 (32-input OR): ≈ 0.4 ns (ghép qua các lát LUT6).
  - Trễ Tầng 1 (32-bit Priority Encode): ≈ 0.8 ns.
  - Trễ MUX + Tầng 2: ≈ 0.9 ns.
  - Tổng trễ logic ≈ 2.1 ns. Thừa khả năng đóng timing trong chu kỳ 5.0 ns (200 MHz) trên Zynq UltraScale+.

---

### 2.3. Module `order_id_tracker.sv` (Bảng Ánh xạ Hủy Lệnh O(1))
- **Vấn đề cần giải quyết:** Khi nhà đầu tư yêu cầu hủy lệnh số `order_id`, làm sao biết lệnh này đang nằm ở ô nhớ BRAM nào mà không phải tìm kiếm O(N)?
- **Kiến trúc mạch:**
  - BRAM Simple Dual-Port 4096 × 12-bit đóng vai trò bảng ánh xạ trực tiếp (Direct Table).
...
| :--- | :---: | :---: | :---: |
| **LUT (Logic)** | ≈ 2.500 | 70.560 | **≈ 3.5%** |
| **FF (Flip-Flops)** | ≈ 3.200 | 141.120 | **≈ 2.3%** |
| **BRAM36K** | **6 khối** | 216 khối | **≈ 2.8%** |
