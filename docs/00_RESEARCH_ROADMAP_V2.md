...
|---|---|---|
| **Cấu trúc dữ liệu LOB** | Nhiều hệ thống (kể cả production) vẫn dùng sorted array/list $\mathcal{O}(N)$ để chèn lệnh theo price-time priority; khi order rate tăng đột biến, latency "rò rỉ" âm thầm mà không hiện trên số liệu trung bình. | Một trường hợp thực tế: hãng đầu tư hạ tầng FPGA/NIC hàng triệu USD nhưng cải thiện đo được gần như bằng không vì bottleneck thật nằm ở cấu trúc $\mathcal{O}(N)$ trong tầng order book. |
...
| **Cấp phát bộ nhớ động** | Insert/cancel liên tục gây phân mảnh bộ nhớ; nếu thiếu cơ chế cấp phát $\mathcal{O}(1)$/wait-free, worst-case phụ thuộc pattern lệnh, phá vỡ tính xác định. | Ngay một đồ án môn học (Columbia CSEE4840) cũng phải tự xây quản lý bộ nhớ theo kiểu page-table để cấp phát động cho nhiều sổ lệnh song song. |
| **Parser giao thức (ITCH/FIX/OUCH)** | Parser cấu hình lại được (P4-style) đã khá chín ở mảng networking tổng quát — làm lại nguyên bản sẽ thiếu novelty. | Đã có SOTA: parser cấu hình động dùng TCAM-SRAM đạt $>80\text{ Gbps}$, độ trễ tối đa $36\text{ ns}$ cho tầng L4; kiến trúc sinh từ P4 đạt $100\text{ Gb/s}$, giảm 45% độ trễ và 40% LUT so với SOTA trước đó. |
| **Memory subsystem / AXI** | Độ trễ round-trip qua PCIe/AXI DMA vẫn ở mức trăm $\text{ns}$–hơn $1\ \mu\text{s}$ do overhead giao thức, không giảm nhiều dù nâng đời chuẩn bus. | Ngành ghi nhận: độ trễ khứ hồi PCIe khả kiến ở phần mềm (DMA, TLB, các lớp giao thức) khoảng $800\text{ ns}$ – hơn $1\ \mu\text{s}$; nâng Gen5 không cải thiện đáng kể. |
...
- **Kiến trúc lõi:** Direct Price-Indexed Array + Hierarchical Bitmap Priority Encoder cho top-of-book $\mathcal{O}(1)$ cố định chu kỳ, thay vì cấu trúc cây phụ thuộc dữ liệu.
...
- Hủy lệnh $\mathcal{O}(1)$ qua direct-map order_id $\rightarrow$ con trỏ node.
...
- **Hardware Traffic Generator (HW-TG) trong PL:** Dùng BRAM/URAM nạp trace ITCH thật, phát lại ở xung nhịp gốc ($250\text{ MHz}$) dồn dập (burst) để đo kiểm.
...
- Đóng timing ở tần số $\ge 200 - 250\text{ MHz}$ trên chip UltraScale+ (phân tích WNS $\ge 0$).

**Phương pháp đo kiểm chuẩn mực IEEE:**
- Đo kiểm hoàn toàn trong PL bằng HW-TG + Dual-timestamp cycle-accurate capture ($\Delta T = T_2 - T_1$).
- Xuất biểu đồ CDF/Histogram với các phân vị P50, P90, P99, P99.99 và Worst-case Max.
- Mục tiêu: Tick-to-trade end-to-end $< 1\ \mu\text{s}$ với cận trên được chứng minh hình thức.
