...
### Bài toán giải quyết:
- **Hạn chế của giải pháp phần mềm:** Các sàn truyền thống chạy phần mềm C++ trên CPU x86. Khi có biến động lớn, hàng triệu lệnh ập vào cùng lúc khiến CPU bị nghẽn (cache thrashing, ngắt OS, tranh chấp khóa bộ nhớ). Độ trễ bị vọt từ $2\ \mu\text{s}$ lên tới $500\ \mu\text{s}$ kèm hiện tượng **giật lag (Jitter)** nghiêm trọng.
- **Giải pháp của Đồ án 1:** Xây dựng một **Bộ vi xử lý phần cứng chuyên dụng (Dedicated FPGA Accelerator)** thay thế CPU:
  ...
  - Đạt độ trễ xác định thuần phần cứng: **$20 - 25\text{ ns}$ (4 – 5 chu kỳ clock tại $200\text{ MHz}$)**.
...
| :--- | :---: | :--- | :--- |
| **`order_id`** | 32-bit | Số nguyên không âm ($0 \dots 2^{32}-1$) | Mã định danh duy nhất của lệnh do hệ thống cấp để theo dõi và hủy lệnh. |
| **`symbol_id`** | 16-bit | Mã định danh cổ phiếu (ĐA1 = 0) | Cố định = 0 ở ĐA1; mở rộng đa mã ở ĐATN. |
| **`price`** | 32-bit | $0 \dots 1023$ (ánh xạ chỉ số BRAM) | Mức giá đặt mua hoặc bán. Ánh xạ trực tiếp thành chỉ số hàng đợi. |
| **`qty`** | 32-bit | $1 \dots 4.294.967.295$ | Khối lượng cổ phiếu/hợp đồng yêu cầu giao dịch. |
...
- **Tổng thời gian: Đúng 5 chu kỳ clock ($25\text{ ns}$ tại $200\text{ MHz}$).**

### Kịch bản 2: Khớp Lệnh (Matching Order)
*Xảy ra khi lệnh MUA có giá $\ge$ Giá bán tốt nhất (Best Ask), hoặc lệnh BÁN có giá $\le$ Giá mua tốt nhất (Best Bid).*
- **Chu kỳ 1:** Ingress Risk Check. Cây Bitmap trả về ngay Best Price đối ứng.
- **Chu kỳ 2:** Đọc `head_ptr` của mức giá Best Price.
- **Chu kỳ 3:** Thực hiện phép toán so khớp:
  $$\text{exec\_qty} = \min(\text{in\_qty}, \text{head\_qty})$$
...
- **Tổng thời gian: 4 – 5 chu kỳ clock.**

### Kịch bản 3: Hủy Lệnh (Cancel Order)
*Xảy ra khi nhận bản tin `action = ACT_CANCEL` mang `order_id` cần hủy.*
- **Chu kỳ 1:** Lấy `order_id` tra vào bảng `order_id_tracker` $\rightarrow$ Nhận con trỏ `target_node`.
...
- **Tổng thời gian: Đúng 4 chu kỳ clock ($20\text{ ns}$ tại $200\text{ MHz}$).**
...
| **Thông lượng xử lý tối đa (Throughput)** | **$\approx 40 - 50$ triệu lệnh/s** | Tính theo công thức $F_{\text{clk}} / \text{Latency} = 200\text{ MHz} / (4\text{–}5\ \text{cycles})$. |
